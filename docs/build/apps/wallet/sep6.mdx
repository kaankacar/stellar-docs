...
If the anchor supports [SEP-38] quotes, it can support deposits that make a bridge between non-equivalent assets. For example, an anchor receives BRL via bank transfer and in return sends USDC (of equivalent value minus fees) to the user on Stellar.
